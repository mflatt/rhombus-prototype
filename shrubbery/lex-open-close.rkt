...
(define (lex/open-close/status in pos status racket-lexer/status)
  (define-values (orig-status inner-status)
    (cond
      [(open-close-tracked? status)
       (values status (open-close-tracked-status status))]
      [else
       (values (open-close-tracked status null 0)
               status)]))
    ...
    [(bar-operator)
     ...
     (cond
       ...
       [(alts? frame)
        ...
        (define-values (close-type close-status) (start-block type status 'more-alts))
        (finish close-type close-status)]
