...
@doc(
  method (ev :: MouseEvent).is_down(sym :: MouseEvent.DownSymbol)
    :: Boolean
  method (ev :: MouseEvent).get_down() :: Set.of(MouseEvent.DownSymbol)
){

 Reports importation about mouse buttons and modified keys that were
 pressed as the time of the mouse event.
...
@doc(
  method (ev :: KeyEvent).is_down(sym :: KeyEvent.DownSymbol) :: Boolean
  method (ev :: KeyEvent).get_down() :: Set.of(KeyEvent.DownSymbol)
){
