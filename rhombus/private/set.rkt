     ...
     (parse-map-by stx
                   (lambda (stx arg-stxes str mapper)
                     (parse-set stx arg-stxes #f
                                (map-by-set-build-id mapper)
                                (map-by-set-build*-id mapper)
                                (map-by-list->set-id mapper)))))))
...
(define-binding-syntax Set.by
  (binding-transformer
   (lambda (stx)
     (parse-map-by stx
                   (lambda (stx arg-stxes str mapper)
                     (define mode #`(#,str #,(map-by-set?-id mapper) values))
                     ...
                     (syntax-parse stx
                       ...
                       [(form-id (~and dot (op |.|)) (~and of-id of) . tail)
                        (parse-annotation-of #'(of-id . tail)
                                             (map-by-set?-id mapper) set-static-infos
                                             1 #f
                                             set-annotation-make-predicate
                                             set-annotation-make-static-info
                                             #'set-build-convert #`(#,(map-by-empty-stx mapper)))]
                       [(_ . tail)
                        (values (annotation-predicate-form (map-by-set?-id mapper)
                                                           set-static-infos)
                                #'tail)]))))))
   ...
   (lambda (stx)
     (parse-map-by stx
                   (lambda (stx arg-stxes str mapper)
                     (syntax-parse stx
                       [(_ . tail)
                        (values (annotation-predicate-form (map-by-mutable-set?-id mapper)
                                                           mutable-set-static-infos)
                                #'tail)]))))))
     ...
     (parse-map-by stx
                   (lambda (stx arg-stxes str mapper)
                     (parse-mutable-set stx #f
                                        (map-by-mutable-set-build-id mapper)))))))
