...
(define-space-clause-syntax export_macro
  (space-clause-transformer
   (lambda (stx)
     (syntax-parse stx
       [(_ id:identifier)
   ...
   (lambda (stx)
     (syntax-parse stx
       [(_ id:identifier (_::block . content))
        (wrap-clause #`(#:meta-namespace id content))]))))
  ...
  (for/fold ([options #hasheq()]) ([option (syntax->list options-stx)])
    (syntax-parse option
      [(_ (#:space_path space-path-name orig-clause))
       ...
       (hash-set options '#:space-path #'space-path-name)]
      [(_ (#:export_macro define-macro))
       (when (hash-ref options '#:define-macro #f)
         (raise-syntax-error #f "multiple export macro names declared" orig-stx #'define-macro))
       (hash-set options '#:define-macro #'define-macro)]
      [(_ (#:meta-namespace name content))
       (when (hash-ref options '#:meta-namespace #f)
         (raise-syntax-error #f "multiple meta namespaces declared" orig-stx #'name))
       (hash-set options '#:meta-namespace (cons #'name #'content))]
