     ...
     (parse-map-by stx
                   (lambda (stx arg-stxes str mapper)
                     (parse-map stx arg-stxes #f
                                (map-by-map-build-id mapper)
                                (map-by-map-pair-build-id mapper)
                                (map-by-list->map-id mapper)))))))
...
(define-binding-syntax Map.by
  (binding-transformer
   (lambda (stx)
     (parse-map-by stx
                   (lambda (stx arg-stxes str mapper)
                     (define mode #`(#,str #,(map-by-map?-id mapper) values))
                     ...
                     (syntax-parse stx
                       ...
                       [(form-id (~and dot (op |.|)) (~and of-id of) . tail)
                        (parse-annotation-of #'(of-id . tail)
                                             (map-by-map?-id mapper) map-static-infos
                                             2 #f
                                             map-annotation-make-predicate
                                             map-annotation-make-static-info
                                             #'map-build-convert #`(#,(map-by-empty-stx mapper)))]
                       [(_ . tail)
                        (values (annotation-predicate-form (map-by-map?-id mapper)
                                                           map-static-infos)
                                #'tail)]))))))
   ...
   (lambda (stx)
     (parse-map-by stx
                   (lambda (stx arg-stxes str mapper)
                     (syntax-parse stx
                       [(_ . tail)
                        (values (annotation-predicate-form (map-by-mutable-map?-id mapper)
                ...
                (build-compound-repetition
                 ...
                 (lambda args
                   (values (quasisyntax/loc stx
                             (hash-copy (#,map-build-id #,@args)))
                           mutable-map-static-infos)))]
               [else
                (wrap-static-info*
                 (quasisyntax/loc stx
                   (hash-copy (#,map-build-id #,@(if (null? argss) null (car argss)))))
                 mutable-map-static-infos)])
             #'tail)]
    [(_ . tail) (values (if repetition?
                            (identifier-repetition-use mutable-map-build-id)
                            mutable-map-build-id)
                        #'tail)]))
     ...
     (parse-map-by stx
                   (lambda (stx arg-stxes str mapper)
                     (parse-mutable-map stx #f
                                (map-by-map-build-id mapper)
                                (map-by-mutable-map-build-id mapper)))))))
