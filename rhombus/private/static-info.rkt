...
(define-for-syntax (static-infos-intersect as bs)
  (cond
    ...
    [(null? (syntax-e bs)) bs]
    [else
     (let ([as (flatten-indirects (syntax->list as))]
           [bs (flatten-indirects (syntax->list bs))])
       (or
        (and as
             bs
             (for/list ([a (in-list as)]
                                       ...
                                       [(b-key b-val)
                                        ...
                                        (let ([key (syntax-local-value* #'a-key static-info-key-ref)])
                                          (cond
                                            ...
                                            [(and (identifier? #'a-val)
                                                  (identifier? #'b-val))
                                             (static-info-identifier-intersect #'a-val #'b-val)]
                                            [else
                                             (static-infos-intersect #'a-val #'b-val)]))]
                                       [_ #f]))]
                                  [_ #f]))]
                        #:when new-val)
               (syntax-parse a
...
(define-for-syntax (static-infos-union as bs)
  (cond
    ...
    [else
     (let ([bs (flatten-indirects (if (list? bs) bs (syntax->list bs)))]
           [as (flatten-indirects (if (list? as) as (syntax->list as)))])
       (cond
         [(not as) (or bs #'())]
         [(not bs) as]
         [else
                                        ...
                                        (let ([key (syntax-local-value* #'a-key static-info-key-ref)])
                                          (list
                                           (cond
                                             ...
                                             [(and (identifier? #'a-val)
                                                   (identifier? #'b-val))
                                              (static-info-identifier-union #'a-val #'b-val)]
                                             [else
                                              (static-infos-union #'a-val #'b-val)])))]
                                       [_ #f])))
                                 (if v
                                     (car v)
                                     #'a-val)]
