...
(provide #%quotes
         ...
         $
         $&
         (for-space rhombus/syntax_pattern_binding
                    _
                    #%parens
                    #%quotes
                    ::))
...
(begin-for-syntax
  ...
  (define-splicing-syntax-class (:esc dotted? any-id?)
    ...
    (pattern (~seq term)
             #:when (or any-id?
                        (not (identifier? #'term))
                        (not (syntax-pattern-binding-id? #'term))
                        (free-identifier=? (in-syntax-pattern-binding-space #'_)
                                           (in-syntax-pattern-binding-space #'term)))))
...
(define-syntax-pattern-binding-syntax _
...
(define-syntax-pattern-binding-syntax #%parens
  (syntax-pattern-binding-prefix-operator
   #'#%parens
   null
   'macro
   (lambda (stx)
     (syntax-parse stx
       [(_ (parens g::syntax-pattern-binding) . tail)
        (values #'g.parsed
                #'tail)]))))

(define-syntax-pattern-binding-syntax #%quotes
  (syntax-pattern-binding-prefix-operator
   #'#%quotes
   null
   'macro
   (lambda (stx)
     (syntax-parse stx
       ...
       [(_ (quotes (group t)) . tail)
        (values (if (eq? (current-syntax-pattern-binding-kind) 'term)
                    #'((~datum t) () () ())
                    #'#f)
                #'tail)]))))

(define-syntax-pattern-binding-syntax ::
  (syntax-pattern-binding-infix-operator
   #'::
   null
   'macro
   (lambda (form1 stx)
         ...
         (with-syntax ([id form1])
           ...
           (define (incompat)
             (raise-syntax-error #f
                                 ...
                                 #'stx-class))
           (define kind (current-syntax-pattern-binding-kind))
           (cond
...
(define-for-syntax (convert-pattern e
  ...
  (define (handle-escape $-id e in-e kind)
    (define parsed
      (parameterize ([current-syntax-pattern-binding-kind kind])
