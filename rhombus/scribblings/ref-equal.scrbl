...
@doc(
 ...
 The @rhombus(==, ~map_by) map configuration can be used with forms like
...
@doc(
 ...
 The @rhombus(===, ~map_by) map configuration can be used with forms
...
@doc(
 ...
 The @rhombus(is_now, ~map_by) map configuration can be used with forms
 like the @rhombus(Map.by) constructor or @rhombus(Map.by, ~annot)
...
@doc(
  operator ((v1 :: Any) is_number_or_object (v2 :: Any)) :: Boolean
  map_by.def 'is_number_or_object'
){

 The @rhombus(is_number_or_object) operator checks whether two values
 are either equal via @rhombus(===) or are two numbers that are
 @rhombus(==). This comparison is primarily intended for comparing
 numbers, and especially via @rhombus(is_number_or_object, ~map_by) as a
 map configuration for equating map keys, where
 @rhombus(is_number_or_object, ~map_by) can be slightly more efficient
 than @rhombus(==).
