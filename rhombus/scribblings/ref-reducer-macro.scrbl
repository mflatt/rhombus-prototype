...
@examples(
  ...
  ~defn:
    reducer.macro 'counted($(r :: reducer_meta.Parsed))':
      let '($wrap, ($(bind && '$id $_'), ...), $pre_clause, $step,
            $break, $final, $finish,
            $si, $data)':
      ...
      reducer_meta.pack(
        'build_return',
        '(count = 0, $bind, ...)',
        pre_clause.unwrap() && 'build_pre_clause',
        'build_inc',
        ...
        '[[count, $id, ...],
          $wrap, $pre_clause, $step, $break, $final, $finish,
          $data]'
      )
    ...
    defn.macro 'build_pre_clause [$_, $_, $pre_clause, $_, $_, $_, $_, $data]':
      '$pre_clause $data'
