...
@title(~tag: "ststic-pict"){Static Picts}
...
@rhombus(Pict.pad) function, when given positive padding, grow a pict's
...
@examples(
  ~eval: pict_eval
  fun antenna(p):    
    let ap = above(circle(~size: 4), line(~dy: 6, ~width: 2), p)
...
@examples(
  ...
  ~repl:
    fun underline(p):
      above(p, line(~dx: p.width))
    beside(~vert: #'baseline, hello, underline(world))
  ~repl:
    fun underline(p):
      above(p, line(~dx: p.width)).refocus(p)
    beside(~vert: #'baseline, hello, underline(world))
  ~repl:
    beside(~vert: #'baseline,
           hello,
           ...
           rectangle(~around: world.pad(~all: 2), ~refocus: world))
)
