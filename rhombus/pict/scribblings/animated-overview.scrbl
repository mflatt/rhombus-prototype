...
An @defterm{animated pict} is based a function that takes a time
...
pict needs a @deftech{time box} that determines how it changes relative
to other animated picts. A time box's @deftech{duration} is analogous to
a bounding box's width or height.
...
in an infinite drawing plane, an animated picture has a rendering for
all time before and after it's time box. A static pict's rendering
...
is relative to the pict's time box, and the time box is shown in gray on
the timeline:
...
Operations like @rhombus(beside), @rhombus(above), and
...
Time-box padding plus composite operations like @rhombus(above) allow
...
combining them with @rhombus(above) makes a composite pict
@rhombus(ping_pong):
...
@examples(  
  ...
  ~defn:
    ...
    def ping_pong = above(early_circ, late_rect)
)
...
@timeline(above(circ.time_pad(~after: 1), rect.time_pad(~before: 1)),
          ~label: "ping_pong",
          [-1, 0, 1, 2])

We could have used @rhombus(overlay) instead of @rhombus(above) to have
...
@timeline(rectangle(~around: overlay(circ.time_pad(~after: 1), rect.time_pad(~before: 1)).pad(~all: 3)),
...
@timeline(rectangle(~around: switch(circ, rect).pad(~all: 3)),
          ~label: "one_at_a_time"
          [-1, 0, 1, 2])
...
At this point, you no doubt wonder about the units of time @math{t} for
an animated pict. Animated picts are especially designed for slide
presentations, where we want a notion of time that is compatible with
both slide-based stepping and real-time animations to transition between
slides. Thus, an increment of @rhombus(1) in @math{t} corresponds to
advancing the slide, and increments between @rhombus(0) and @rhombus(1)
are transition points. More abstractly, we refer to each increment of
@math{t} by @rhombus(1) as an @deftech{epoch}. A pict's duration is
always measured in integer epochs.

When an epoch advances (i.e., when the space bar is hit to advance the
slide), then animation functions receive a sequence of numbers between
@rhombus(0) and @rhombus(1) to represent the intermediate transition
points between the old slide and the new one. The rate at which those
intermediate results are used is based the epoch's @deftech{extent},
which is measured in seconds. An animated pict can have a different
extent for each of its epochs. A static pict's epochs all have extent
@rhombus(0), meaning that it doesn't need to draw any transitions on the
way out of the epoch.

If we stretch the conceptual pict timeline to match real time then might
look something like this:

@centered(stretched_timeline)

The red dots to start epochs become ovals representing long and varying
pauses while a slide is shown. The right end end of a red oval is the
start of a transition to a new slide, and the new slide arrives fully at
the next red oval. The example timeline depicts a pict with different
extents for epochs @rhombus(0) through @rhombus(2) within its time box,
so the spaces between the red ovals vary accordingly. Epoch @rhombus(1)
has an especially long-running transistion (i.e., a large extent).

To create an animated picture with a non-@rhombus(0) extent, use the
@rhombus(animated) constructor:
...
@examples(
  ...
  ~defn:
    ...
    def together = above(long_circ, late_rect)
)

@(def long_circ = switch(circ, circ))

@timeline(above(long_circ, rect.time_pad(~before: 1)),
          ~label: "together",
          [-1, 0, 1, 2])
...
@examples(
  ...
  ~defn:
    ...
    def together_try = above(two_fade, late_rect)
)

@(def two_fade = switch(fade_in_out, fade_in_out))

@timeline(above(two_fade, rect.time_pad(~before: 1)),
          ~label: "together_try",
...
animated pict is common enough that it available as
@rhombus(Pict.sustain). In fact, sustaining an animated pict is the
right choice so often that it is the default way that operations like
@rhombus(overlay) and @rhombus(above) extend the duration of a pict to
match others. In other words, we don't have to manually force
@rhombus(fade_in_out) and @rhombus(late_rect) to have the same duration,
and @rhombus(above) will sustain @rhombus(fade_in_out) to make its
duration match @rhombus(late_rect):

@examples(
  ~eval: pict_eval
  ~defn:
    def together_fade = above(fade_in_out, late_rect)
)

@(def together_fade = above(fade_in_out, rect.time_pad(~before: 1)))
...
picts, functions like @rhombus(overlay) and @rhombus(above) accept a
@rhombus(~duration) optional argument that can be @rhombus(#'pad) instead
of @rhombus(#'sustain) to make them normalize durations by padding
