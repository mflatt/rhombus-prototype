...
(require (for-syntax racket/base
         ...
         (only-in rhombus/private/syntax-pattern-binding
         ...
         (only-in rhombus/meta
                  ...
                  class_clause interface_clause entry_point syntax_pattern_binding)
         (only-in "rhombus.rhm"
                  rhombusblock_etc
                  [rhombus one-rhombus])
...
(define-for-syntax (extract-introducer stx)
  (syntax-parse stx
    ...
    [(group syntax_pattern_binding . _) in-syntax-pattern-binding-space]
...
(define-for-syntax (extract-space-name stx)
  (syntax-parse stx
    #:literals (impo expo modpath annot repet reducer for_clause class_clause interface_clause entry_point
                     bind syntax_pattern_binding rhombus-syntax)
    #:datum-literals (parens group op)
    [(group impo . _) 'impmod]
    ...
    [(group syntax_pattern_binding . _) 'syntax_pattern_binding]
    [(group bind . _) 'bind]
...
(define-for-syntax (extract-kind-str stx)
  (syntax-parse stx
    #:literals (defn decl expr impo expo modpath annot repet reducer
                 ...
                 syntax_pattern_binding interface)
    ...
    [(group syntax_pattern_binding . _) "syntax pattern binding"]
